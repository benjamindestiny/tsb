...
const YOUR_OPAY_NAME = "William Ruth Ashomen"; 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-purple-900 to-gray-900">
        ...
        <div className="max-w-4xl mx-auto px-4 md:px-6 py-4 flex items-center justify-between flex-wrap gap-3">
          <div className="flex items-center gap-2">
            <Link to="/" className="flex items-center gap-2">
              <img src="/logo.png" alt="TSB" className="w-10 h-10 object-contain" />
          ...
          {loggedInUser ? (
            <Link to="/dashboard" className="flex items-center gap-2 text-sm text-gray-300 hover:text-white">
              <div className="w-7 h-7 bg-gradient-to-br from-purple-500 to-indigo-500 rounded-full flex items-center justify-center text-white text-xs font-bold">
                {(loggedInUser.displayName || loggedInUser.username || "?").charAt(0).toUpperCase()}
              </div>
              <span>@{loggedInUser.username}</span>
            </Link>
          ) : (
            <Link to="/login" className="text-sm text-gray-400 hover:text-white">Are you a creator? Login</Link>
          )}
        </div>
      </div>
      ...
      <div className="max-w-2xl mx-auto px-4 md:px-6 py-8 md:py-12 text-center">
        ...
        <h1 className="text-3xl font-bold text-white mb-2">{creator.displayName || creator.username}</h1>
        <p className="text-gray-400 mb-2">@{creator.username}</p>
        {creator.bio && <p className="text-gray-300 mb-6">{creator.bio}</p>}

        <p className="text-gray-400 mb-8 italic">"Enjoy my content? Tap The Support Button ☕"</p>

        {!showPayment ? (
          <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => setShowPayment(true)} className="px-6 md:px-10 py-4 bg-gradient-to-r from-purple-500 to-indigo-500 text-white font-bold rounded-2xl text-xl shadow-lg">
            ☕ Support Me
          </motion.button>
        ) : showOpayDetails ? (
          /* OPAY DETAILS SCREEN */
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm border-2 border-purple-500/50 rounded-2xl p-6 text-left">
            <h3 className="text-white font-semibold text-lg mb-4 text-center">📱 Send via OPay</h3>

            <div className="bg-purple-500/10 border border-purple-500/30 rounded-xl p-4 mb-4 text-center">
              <p className="text-gray-400 text-sm mb-1">Send exactly this amount:</p>
              <p className="text-green-400 font-bold text-3xl">₦{Number(supportAmount).toLocaleString()}</p>
            </div>
            ...
            <div className="bg-yellow-500/10 border border-yellow-500/30 rounded-xl p-3 mb-4">
              <p className="text-yellow-400 text-sm text-center">
                ⚠️ Open your OPay app now and send <strong>₦{Number(supportAmount).toLocaleString()}</strong> to <strong>{YOUR_OPAY_NUMBER}</strong>
              </p>
            </div>

            <button onClick={confirmPaymentSent} className="w-full py-3 bg-green-500 hover:bg-green-600 text-white font-semibold rounded-xl transition-colors">
              ✅ I've Sent the Money
            </button>

            <button onClick={() => { setShowOpayDetails(false); setShowPayment(false); }} className="w-full mt-2 py-2 text-gray-400 text-sm hover:text-white">Cancel</button>
          </motion.div>
        ) : (
          /* PAYMENT FORM */
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-gray-800/50 backdrop-blur-sm border border-gray-700 rounded-2xl p-6 text-left">
            <h3 className="text-white font-semibold text-lg mb-4 text-center">Send Support via OPay</h3>

            <div className="mb-4">
              <label className="block text-sm text-gray-400 mb-2">Amount (₦)</label>
              <div className="flex gap-2 mb-2 flex-wrap">
                {quickAmounts.map((amt) => (
                  <button key={amt} onClick={() => setSupportAmount(amt.toString())} className={`px-3 py-1 rounded-lg text-sm ${supportAmount === amt.toString() ? "bg-purple-500 text-white" : "bg-gray-700 text-gray-300 hover:bg-gray-600"}`}>
                    ₦{amt.toLocaleString()}
                  </button>
                ))}
              </div>
              <input type="number" value={supportAmount} onChange={(e) => setSupportAmount(e.target.value)} placeholder="Custom amount" min={3000} className="w-full px-4 py-3 bg-gray-700/50 border border-gray-600 rounded-xl text-white focus:outline-none focus:border-purple-500" />
            </div>
            ...
            <button onClick={() => setShowPayment(false)} className="w-full mt-2 py-2 text-gray-400 text-sm hover:text-white">Cancel</button>
          </motion.div>
        )}
      </div>
